...
class CNN_Data(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = np.load(self.Data_dir + self.Data_list[idx] + '_T1.npy').astype(np.float32)
...
class FCN_Data(CNN_Data):
    ...
    def __getitem__(self, idx):
        ...
        if self.stage == 'train' and not self.whole:
            #data = np.load(self.Data_dir + self.Data_list[idx] + "_T1" + '.npy', mmap_mode='r').astype(np.float32)
            data = np.load(self.Data_dir + self.Data_list[idx] + '_T1.npy', mmap_mode='r').astype(np.float32)
            ...
            #data = np.load(self.Data_dir + "T1_MNI.nii" + '.npy').astype(np.float32)
#            data = np.load(self.Data_dir + self.Data_list[idx] + '.npy').astype(np.float32)
            data = np.load(self.Data_dir + self.Data_list[idx] + '_T1.npy',
...
class CNN_MLP_Data(Dataset):
    def __init__(self, Data_dir, exp_idx, stage, seed=1000):
        ...
        self.Data_list, self.Label_list, self.demor_list = read_csv_complete('/data1/home/yanwenjie/PycharmProjects/brain2020-master/lookupcsv/exp0/train.csv')
        #self.Data_list, self.Label_list, self.demor_list = read_csv_complete(path)
        self.risk_list = [np.load(Data_dir + filename + '_T111'+'.npy') for filename in self.Data_list]
...
if __name__ == "__main__":
    data = CNN_MLP_Data(Data_dir='./DPMs/cnn_exp1/output4/', exp_idx=1, stage='train')
    #data = CNN_MLP_Data(Data_dir='./data3/home/yan1/test/brain2020-master/', exp_idx=1, stage='train')
