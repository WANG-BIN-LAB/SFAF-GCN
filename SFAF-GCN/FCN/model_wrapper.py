...
class CNN_Wrapper:
    def __init__(self,
        ...
        self.model = _FCN(fil_num=fil_num, drop_rate=drop_rate).cuda()
        self.prepare_dataloader(batch_size, balanced, Data_dir)
        self.checkpoint_dir = '/data1/home/yanwenjie/PycharmProjects/brain2020-master/checkpoint_dir/{}_exp{}/'.format(self.model_name, exp_idx)
        ...
        with torch.no_grad():
            for stage in ['train', 'valid', 'test']:
                ...
                if stage in ['AIBL', 'NACC', 'FHS']:
                    Data_dir = Data_dir.replace('NC', stage)
    ...
    def gen_features(self):
        ...
        self.model.train(False)
        self.feature_dir = './Features/{}_exp{}/'.format(self.model_name, self.exp_idx)
...
class FCN_Wrapper(CNN_Wrapper):
    ...
    def __init__(self, fil_num,
        ...
        #def __init__(self, num, p, Data_dir, exp_idx, stage, idx, seed=1000):
        #self.model = _FCN(num=fil_num, p=drop_rate).cuda()
        self.model = _FCN(p=drop_rate, num=fil_num, stage='train', seed=1000,Data_dir=self.Data_dir, exp_idx=self.exp_idx).cuda()
        # self.features = []
        self.prepare_dataloader(batch_size, balanced, Data_dir)
    ...
    def valid_model_epoch(self):
        ...
        with torch.no_grad():
            ...
            for idx, (inputs, labels) in enumerate(self.valid_dataloader):
                inputs, labels = inputs.cuda(), labels.cuda()
                DPM = self.fcn(inputs, stage='get_features')
    ...
    def test_and_generate_DPMs(self):
        ...
        with torch.no_grad():
            for stage in ['train', 'valid', 'test']:
                ...
                for idx, (inputs, labels) in enumerate(dataloader):
                    inputs, labels = inputs.cuda(), labels.cuda()
                    DPM = self.fcn(inputs, stage='get_features').cpu().numpy().squeeze()
                    np.save(self.DPMs_dir + filenames[idx] + '.npy', DPM)
                    DPMs.append(DPM)
                    Labels.append(labels)
...
class MLP_Wrapper_A(CNN_Wrapper):
    ...
    def train(self, lr, epochs):
        ...
        for self.epoch in range(epochs):
            ...
            valid_matrix = self.valid_model_epoch()
            #print('{}th epoch validation confusion matrix:'.format(self.epoch), valid_matrix, 'eval_metric:', "%.4f" % self.eval_metric(valid_matrix))
            self.save_checkpoint(valid_mat-rix)
        #print('Best model saved at the {}th epoch:'.format(self.optimal_epoch), self.optimal_valid_metric, self.optimal_valid_matrix)
        return self.optimal_valid_metric
